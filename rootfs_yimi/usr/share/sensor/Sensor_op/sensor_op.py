...
class sow_sensor():
    ...
    def get_sow_data(self):
        ...
        while count :
            ...
            if (len(sow_data) == 17) :
                ...
                if (sow_data[0] != 0xff) or \
                        (sow_data[1] != 0xfc) or \
                        (sow_data[16] != 0xfd):
                    continue

                for sow_data_index in range(len(sow_data)):
                    tmp_char = tmp_char + str(hex(sow_data[sow_data_index])) + " "

                config.contentSource = tmp_char
    ...
    def calculate_sow_data(self, flag):
        ...
        server_sow_file1 = "server_data_file1.txt"
        server_sow_file2 = "server_data_file2.txt"

        tractor_sow_file1 = "tractor_data_file1.txt"
        tractor_sow_file2 = "tractor_data_file2.txt"

        self.lock.release()
        if (flag == 0):
            if (len(sow_result_new) > 0) :

                server_sow_data1 = file_read(server_sow_file1)
                server_sow_data2 = file_read(server_sow_file2)

                if (server_sow_data1 == 0) and (server_sow_data2 == 0):
                    file_write(server_sow_file1, sow_result_new[3])
                    file_write(server_sow_file2, sow_result_new[4])

                    return sow_result_new

                else :
                    sow_result = copy.deepcopy(sow_result_new)
                    sow_result[3] = sow_result_new[3] - server_sow_data1
                    sow_result[4] = sow_result_new[4] - server_sow_data2
                    file_write(server_sow_file1, sow_result_new[3])
                    file_write(server_sow_file2, sow_result_new[4])
            ...
            if (len(sow_result_new) > 0) :
                tractor_sow_data1 = file_read(tractor_sow_file1)
                tractor_sow_data2 = file_read(tractor_sow_file2)

                if (tractor_sow_data1 == 0) and(tractor_sow_data2 == 0):
                    file_write(tractor_sow_file1, sow_result_new[3])
                    file_write(tractor_sow_file2, sow_result_new[4])

                    return sow_result_new

                else :
                    sow_result = copy.deepcopy(sow_result_new)
                    sow_result[3] = sow_result_new[3] - tractor_sow_data1
                    sow_result[4] = sow_result_new[4] - tractor_sow_data2
